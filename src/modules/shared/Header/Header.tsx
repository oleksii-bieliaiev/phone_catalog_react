...
const PATHS_WITH_SEARCH = ['/phones', '/tablets', '/accessories', '/favorites'];
...
export const Header: FC = () => {
  ...
  const [searchPlaceholder, setSearchPlaceholder] = useState(PLACEHOLDER_TEXT);
  ...
  const shouldShowSearch = useMemo(
    () => PATHS_WITH_SEARCH.includes(location.pathname),
    [location.pathname],
  );
  ...
  const themeButtonText = theme === 'light' ? 'on' : 'off';
  ...
  const handleSearchBlur = useCallback(() => {
    setSearchPlaceholder(PLACEHOLDER_TEXT);
  }, []);
